...
def probability_error(c, target_P, mean, stDev, GT, OneSided):
    """
    Function for the Secant method. It calculates the difference between the actual probability and the target.
    """
    if OneSided:
...
def main():
    """
    This program interacts with the user to compute:
    1. P(x < c) or P(x > c) given c.
    2. c given P(x < c) or P(x > c) using the Secant method.
    """
    Again = True
    yesOptions = ["y", "yes", "true"]
    while Again:
        response = input(
            "Are you specifying c and solving for P, or specifying P and solving for c? (Enter 'c' or 'p'): ").strip().lower()
