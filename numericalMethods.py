...
def Probability(PDF, args, c, GT=True):
    """
    This function calculates the probability that x is > c or < c depending on GT.
    Step 1: Unpack args into mu and stDev.
    Step 2: Compute lower and upper limits for Simpson's Rule.
    Step 3: Call Simpson with GPDF and limits.
    Step 4: Return the calculated probability.

    :param PDF: The probability density function to be integrated.
    :param args: A tuple with (mean, standard deviation).
    :param c: The threshold value for the probability question.
    :param GT: Boolean deciding if we want probability x > c (True) or x < c (False).
    :return: Probability value.
    """
    mu, sig = args
    lhl = mu - 5 * sig  # Lower limit for integration
    rhl = c  # Upper limit

    # Ensure Simpson receives a function with one argument
    p = Simpson(lambda x: PDF((x, mu, sig)), (lhl, rhl))
...
def Simpson(fn, limits, N=100):
    """
    Executes the Simpson 1/3 rule for numerical integration.

    :param fn: Function to integrate, which takes a single variable x.
    ...
    """
    lhl, rhl = limits  # Lower and upper integration limits
    m = N + 1 if N % 2 == 1 else N  # Ensure an even number of intervals
    h = (rhl - lhl) / m  # Step size
